...
class OSMImporterWebSocket(WebSocket):
    ...
    def build(self, data):
        ...
        try:
            ...
            self.sendMessage("Zipping is done")
            
            print("Run randomTrips.py")
            
            # IMPORTANT! config randomTrips here
            subprocess.call([
                "python", "sumo/sumo_utils/tools/randomTrips.py", 
                "-n", f"{self.outputDir}/osm.net.xml",
                "-o", f"{self.outputDir}/trips.trips.xml",
                "-r", f"{self.outputDir}/routes.rou.xml",
                # "-e", "50"
                ])
            
            print("Finish randomTrips.py")
